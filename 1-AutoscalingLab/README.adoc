...
----
+
As in the example output, `ResourcesReady` 's message field shows the static number of running replicas out of configured number of replicas. If the `status` reports that the Application is not ready even after a considerate amount of time, check the application's log, by running `oc logs deployment/autoscaling-liberty-app`.
...
----
+
Compared to the last status conditions output, `ResourcesReady` 's message field now outputs the number of running replicas only without the desired number of replicas. It will report `ResourcesReady` is true when the currently running number of pods is within the range of `maxReplicas` and `minReplicas`. If the `status` reports that the Application is not ready for long, check the log.
...
+
Compared to the last status conditions, `ResourcesReady` 's message now shows the number of running replicas only without the desired number of replicas. It will report `ResourcesReady` is true when the currently running number of pods is within the range of `maxReplicas` and `minReplicas`. If any type under `status.conditions` section reports that the Application is not ready, check the pod's log.
