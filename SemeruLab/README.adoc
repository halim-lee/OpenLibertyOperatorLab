...
11. The operator will instantly detect when the application image is modified or updated. Then the operator will create a new set of JIT Server pods for the new application pods. Under `spec` field, modify `replicas` to 2 and `applicationImage` field to another image. Notice the image's digest value has changed. Please note `replicas` field under `semeruCloudCompiler` should stay the same.
+
[source,sh]
----
