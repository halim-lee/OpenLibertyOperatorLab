...
4. Create a YAML file called `liberty-semeru.yaml` with the following content:
+
[source,yaml]
----
...
7. Check semeru related properties in the status section as well.
+
[source,sh]
----
...
8. You can check what resources are managed by the operator through a command.
+
[source,sh]
----
...
9. Check the logs of JIT server pods. Use the pod name using the output above. For example, the pod name will start with `semeru-enabled-sample-semeru-compiler-1`...
+
[source,sh]
----
...
10. Check the Liberty application log to ensure the connection with JIT Server. Use the pod name using the output above. For example, the pod name will start with `semeru-enabled-sample`...
+
[source,sh]
----
...
11. The operator will instantly detect when the application image is modified or updated. Then the operator will create a new set of JIT Server pods for the new application pods. Under `spec` field, modify `replicas` to 2 and `applicationImage` field to another image. Please note `replicas` field under `semeruCloudCompiler` should stay the same.
+
[source,sh]
----
...
12. Check the status of managed resources. You may have to wait until the Liberty app deployment's pods are all updated to the new image.
+
[source,sh]
----
oc get all -l app.kubernetes.io/part-of=semeru-enabled-sample
...
13. Check the logs of JIT server pods. 
+
[source,sh]
----
...
14. You can also detect the changes in OpenLibertyApplication instance's status report as well.
+
[source,sh]
----
oc get OpenLibertyApplication semeru-enabled-sample -ojson | jq '.status.semeruCompiler, .status.references'
...
16. Check the instance's status sections.
+
[source,sh]
----
...
image:images/ola.png[,500]
+
Under `spec` field, modify `replicas` to 2 and `applicationImage` field to another image. Please note `replicas` field under `semeruCloudCompiler` should stay the same.
+
[source,yaml]
----
spec:
