...
2. Create a YAML file called `knative-enabled-liberty.yaml` with the following content:
+
[source,yaml]
----
...
[source,sh]
----
oc apply -f knative-enabled-liberty.yaml
----
The operator will create a Knative Service resource which manages the entire life cycle of a workload. Non-Knative resources, such as Service, Route and Deployment, will not be created.

4. Check the status of the OpenLibertyApplication instance by running:
+
[source,sh]
----
oc get OpenLibertyApplication knative-enabled-liberty -ojson | jq '.status.conditions'
----
...
[source,sh]
----
oc edit OpenLibertyApplication knative-enabled-liberty
----
...
5. Change the OpenLibertyApplication instance to `knative-enabled-liberty` under *Name* field. Set replicas to 1.
+
...
8. Click `OpenLibertyApplication` tab and select `knative-enabled-liberty` instance.
+
