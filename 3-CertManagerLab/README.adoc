...
4. Get the application pod name by running:
+
[source,sh]
----
...
5. Get the URL allocated by the application instance. For example: `https://certificate-liberty-app-test-namespace.apps.was-education-cluster.cp.fyre.ibm.com`.
+
[source,sh]
----
...
8. Edit OpenLibertyApplication in `liberty-certificate.yaml` to add the certificate secret reference to the route. Add `route` field under `spec` field.
+
[source,yaml]
----
spec:
...
12. Revisit the application page on your browser. Check the certificate again. The certificate is still issued to `certificate-liberty-app.test-namespace.svc` but is now issued by `openshift-service-serving-signer`.
+
...
14. Edit OpenLibertyApplication in `liberty-certificate.yaml` to update certificate reference for the route. Edit `route` field under `spec` field as the following:
+
[source,yaml]
----
spec:
...
16. Edit OpenLibertyApplication in `liberty-certificate.yaml` to disable automatic generation of certificates by setting `.spec.manageTLS` field to false:
+
[source,yaml]
----
spec:
...
18. Please delete the OpenLibertyApplication instance and TLS secrets to clean up the resources.
+
[source,sh]
----
oc delete OpenLibertyApplication/certificate-liberty-app
oc delete --field-selector type=kubernetes.io/tls
----
