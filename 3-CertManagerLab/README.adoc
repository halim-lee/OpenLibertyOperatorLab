...
----
+
As in the example output, `ResourcesReady` 's message field shows the number of running replicas out of configured number of replicas. If any type under status conditions section reports that the Application is not ready even after a considerate amount of time, check the application's log, by running `oc logs deployment/certificate-liberty-app`. `svcCertSecretName` field shows the generated certificate secret name that the application uses.
...
[source,sh]
----
oc get secrets --field-selector type=kubernetes.io/tls -n $NAMESPACE | grep certificate-liberty-app
----
+
