...
.*Environment setup: Logging into the cluster and setting namespace*
[%collapsible]
=====
...
8. Edit OpenLibertyApplication to add the certificate secret reference to the route. Add `route` field under `spec` field.
+
[source,sh]
----
...
14. Edit OpenLibertyApplication to update certificate reference for the route.
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app
...
16. Edit the OpenLibertyApplication instance to disable automatic generation of certificates by setting `.spec.manageTLS` field to false. Run the command: 
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app
