...
10. Check the status of HorizontalPodAutoscaler resource to see if scaling is working as expected. You may have to wait for the resource to be ready.
+
[source,sh]
----
...
+
When you click on the HPA resource and scroll down to see the status condition, you should see `AbleToScale` and `ScalingActive` type reporting `True`. `ScalingLimited` type may report `True` saying that the desired replica count is less than the minimum replica count. That is okay for this lab, since it is reporting there can be only 1 replica running. These report that the workload is being scaled with desired resource utilization.
+
