...
----
+
Verify that a TLS secret is also created with the issuer. Below command will grab secrets with name `certificate-liberty-app-svc`, which is liberty instances's name + "-svc".
+
[source,sh]
----
kubectl get secrets -n $NAMESPACE | awk '{if ($1 ~ "certificate-liberty-app-svc") print $0}'
----
+
The output should be similar to the following:
+
[source,log]
----
certificate-liberty-app-svc-tls-cm        kubernetes.io/tls                     3      35s
----
+
This shows the default secret generated by the Certificate Manager, with `-cm` suffix.
...
[source,sh]
----
oc get secret certificate-liberty-app-svc-tls-ocp
----
Then you should see similar to the following:
+
[source,yaml]
----
NAME                                  TYPE                DATA   AGE
certificate-liberty-app-svc-tls-ocp   kubernetes.io/tls   2      3m38s
----
