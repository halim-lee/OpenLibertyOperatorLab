...
3. Create a YAML file called `liberty-certificate.yaml` with the following content:
+
[source,yaml]
----
...
[
  {
    "lastTransitionTime": "2023-08-25T14:09:00Z",
    "status": "True",
    "type": "Reconciled"
  },
  {
    "lastTransitionTime": "2023-08-25T14:09:27Z",
...
6. Verify that a Certificate Authority (CA) `Issuer` instance and a corresponding secret are successfully created in your namespace.
+
[source,sh]
----
...
----
+
This shows the default secret generated by the Certificate Manager, with `svc-tls-cm` suffix appended to the OpenLibertyApplication instance's name.

7. Since the operator is running on OpenShift Container Platform for the lab, we can also explore Red Hat OpenShift service CA as well. This method is the default for the operator and is the simplest way to generate certificates without the help of Certificate Manager. Edit the OpenLibertyApplication instance to force use of the Red Hat OpenShift service CA by configuring `.spec.service` field: 
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app
----
Then edit `service` field under `spec` field as the following:
+
[source,yaml]
----
  service:
...
----
+
This configuration adds annotation to use Red Had OpenShift CA. It will ensure `tls.crt` and `tls.key` files are mounted to the pod and Red Hat OpenShift CA certificate is in the /var/run/secrets/kubernetes.io/serviceaccount/service-ca.crt file.

8. Run the following to see if the secret is successfully created:
+
[source,sh]
----
oc get secrets --field-selector type=kubernetes.io/tls -n $NAMESPACE
...
----
+
Compared to the last TLS secrets output, there is a secret with `svc-tls-ocp` suffix appended to the OpenLibertyApplication instance's name.

9. Let us consider a scenario where you already have a CA certificate ready. Instead of having the operator create its own CA for issuing service certificates, you can bring your own CA certificate. Create a YAML file called `cert-secret.yaml` with the following content:
+
[source,yaml]
----
apiVersion: v1
...
10. Set the application instance to grab custom secret for the application. 
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app
----
Then add `route` field under `spec` field as the following:
+
[source,yaml]
----
  route:
...
11. Visit external route of the application.
+
[source,sh]
----
oc get routes -n $NAMESPACE
----
Then you will get similar output as the following:
+
[source,yaml]
----
NAME                      HOST/PORT                                                                            PATH   SERVICES                  PORT       TERMINATION   WILDCARD
certificate-liberty-app   certificate-liberty-app-user2-namespace.apps.was-education-cluster.cp.fyre.ibm.com          certificate-liberty-app   9443-tcp   reencrypt     None
----
+
Visit the host and check the certificate of the page. The Common Name will be chaged to "ibm.com", which is specified in the custom TLS secret.
+
image:images/cert-page.png[,900]

12. Edit the OpenLibertyApplication instance to disable automatic generation of certificates by setting `.spec.manageTLS` field to false. Run the command: 
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app
...
7. Select `certificate-liberty-app` instance. At the bottom, you will see *Status Conditions* section, which gives you detail on status conditions of the managed resources and the application instance.
+
