...
4. Create a YAML file called `liberty-certificate.yaml` with the following content:
+
[source,yaml]
----
...
6. Check the status of the OpenLibertyApplication instance and its service certificate secret name by running:
+
[source,sh]
----
...
certificate-liberty-app-78f8796974-4qjr8   1/1     Running   0          96s
----
+
Use the pod name from the output, run another command to get Open Liberty logs:
+
[source,sh]
----
oc logs <pod_name>
...
8. Get the URL allocated by the application instance. For example: `https://certificate-liberty-app-test-namespace.apps.was-education-cluster.cp.fyre.ibm.com`.
+
[source,sh]
----
...
[
  {
    ...
    "scope": "External",
    "type": "Application",
    "uri": "https://certificate-liberty-app-test-namespace.apps.was-education-cluster.cp.fyre.ibm.com"
  }
]
----
+
...
9. Check the certificate of the page. It will say `Not Secure` because the certificate is self-signed. You should see that the certificate is issued to *.apps.<OCP-cluster-name>.cp.fyre.ibm.com and issued by ingress-operator. These are the default values when a certificate is not specified.
+
image:images/default-certificate.png[,900]

10. Verify that a Certificate Authority (CA) `Issuer` instance and a corresponding secret are successfully created in your namespace.
+
[source,sh]
----
...
olo-ca-tls                            kubernetes.io/tls   3      21d
----
+
...
11. Edit OpenLibertyApplication to add the certificate secret reference to the route. Add `route` field under `spec` field.
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app -n $NAMESPACE
----
+
[source,yaml]
----
...
12. Revisit the application page on your browser. Check the certificate again. Now the certificate is issued to `certificate-liberty-app.test-namespace.svc` and issued by `Open Liberty Operator`. This is the default self-signed certificate that the Open Liberty Operator provides with Cert Manager.
+
image:images/default-route-certificate.png[,900]

13. Since the operator is running on OpenShift Container Platform for the lab, we can also explore Red Hat OpenShift service CA as well. This method is the default for the operator and is the simplest way to generate certificates without the help of Certificate Manager. Edit the OpenLibertyApplication instance to force use of the Red Hat OpenShift service CA by configuring `.spec.service` field: 
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app
----
Then edit `service` and `route` field under `spec` field as the following:
+
[source,yaml]
----
spec:
...
----
+
This configuration adds annotation to use Red Had OpenShift CA. It will ensure `tls.crt` and `tls.key` files are mounted to the pod and Red Hat OpenShift CA certificate is in the /var/run/secrets/kubernetes.io/serviceaccount/service-ca.crt file. By updating the `certificateSecretRef` field, the application route will load the new certificate.

14. Run the following to see if the secret is successfully created:
+
[source,sh]
----
oc get secrets --field-selector type=kubernetes.io/tls -n $NAMESPACE
...
15. Revisit the application page on your browser. Check the certificate again. The certificate is still issued to `certificate-liberty-app.test-namespace.svc` but is now issued by `openshift-service-serving-signer`.
+
image:images/ocp-route-certificate.png[,900]

16. Let us consider a scenario where you already have a CA certificate ready. Instead of having the operator create its own CA for issuing service certificates, you can bring your own CA certificate. Create a YAML file called `cert-secret.yaml` with the following content:
+
[source,yaml]
----
apiVersion: v1
...
----
+
Note that the secret's name is `olo-custom-ca-tls`. This way, the operator knows that a custom CA certificate exists and reissues a certificate for the service by using the provided CA.
+
...
17. Edit OpenLibertyApplication to update certificate reference for the route.
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app
----
Then edit `route` field under `spec` field as the following:
+
[source,yaml]
----
spec:
...
18. Revisit the application page on your browser. Check the certificate again. The certificate is now issued to `ibm.com` and issued by `ibm.com`.
+
image:images/custom-certificate.png[,900]

19. Edit the OpenLibertyApplication instance to disable automatic generation of certificates by setting `.spec.manageTLS` field to false. Run the command: 
+
[source,sh]
----
oc edit OpenLibertyApplication certificate-liberty-app
...
20. Check the status of the OpenLibertyApplication instance again by running:
+
[source,sh]
----
oc get OpenLibertyApplication certificate-liberty-app -ojson | jq '.status.conditions, .status.references'
...
21. Please delete the OpenLibertyApplication instance to clean up the resources.
+
[source,sh]
----
...
7. Scroll to the botton of the YAML file. As in the example, `message` field shows the number of running replicas out of configured number of replicas. If the `status` reports that the Application is not ready, check the pod's log. `svcCertSecretName` field shows the generated certificate secret name that the application uses.
+
...
+
image:images/topology-logs.png[,900]
+
Wait for the `CWWKF0011I` log to make sure the Liberty server started successfully:
+
[source,log]
...
9. Go back to Topology page. Access the application's external route.
+
...
11. Verify that a Certificate Authority (CA) `Issuer` instance and its corresponding secret are successfully created in your namespace. Click `Search` tab on the left and search for resources `Secret` and `Issuer`. For `Secret`, filter type to TLS. Then you should see the following.
+
...
12. Go back to Topology page. Edit OpenLibertyApplication to add the certificate secret reference to the route. Add `route` field under `spec` field.
+
[source,yaml]
----
spec:
...
13. Revisit the application page on your browser. Check the certificate again. Now the certificate is issued to `certificate-liberty-app.test-namespace.svc` and issued by `Open Liberty Operator`. This is the default self-signed certificate that the Open Liberty Operator provides with Cert Manager.
+
image:images/default-route-certificate.png[,900]
...
15. Try searching for the TLS secrets again. Compared to the last TLS secrets output, there is another secret with `svc-tls-ocp` suffix appended to the OpenLibertyApplication instance's name.
+
image:images/search-secret-new.png[,700]

16. Revisit the application page on your browser. Check the certificate again. The certificate is still issued to `certificate-liberty-app.test-namespace.svc` but is now issued by `openshift-service-serving-signer`.
+
image:images/ocp-route-certificate.png[,900]

17. Let us consider a scenario where you already have a CA certificate ready. Instead of having the operator create its own CA for issuing service certificates, you can bring your own CA certificate. Create a secret by clicking `Secrets`, `Create` and `From YAML`. Create a secret with the following content:
+
[source,yaml]
----
apiVersion: v1
...
18. Edit OpenLibertyApplication to update certificate reference for the route:
+
[source,yaml]
----
spec:
...
20. Edit the OpenLibertyApplication instance to disable automatic generation of certificates by setting `.spec.manageTLS` field to false: 
+
[source,yaml]
----
spec:
