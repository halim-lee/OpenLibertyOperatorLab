...
8. Edit OpenLibertyApplication in `knative-enabled-sample.yaml` to update application image to newer image. Edit `applicationImage` field under `spec` field:
+
[source,yaml]
----
spec:
...
14. Edit OpenLibertyApplication in `knative-enabled-sample.yaml` to disable Knative configuration. Locate `createKnativeService` field under `spec` field and change its value to false:
+
[source,yaml]
----
spec:
